...
PROXY_COUNTRIES = ["Россия", "Украина", "США", "Казахстан", "Англия", "Германия", "Канада", "Нидерланды", "Великобритания", "Австрия"]
...
async def init_db():
    ...
    await DB_CONN.execute("CREATE TABLE IF NOT EXISTS users(user_id INTEGER PRIMARY KEY, username TEXT, first_name TEXT)")
    await DB_CONN.execute("CREATE TABLE IF NOT EXISTS admins(user_id INTEGER PRIMARY KEY)")
    await DB_CONN.execute("CREATE TABLE IF NOT EXISTS bans(user_id INTEGER PRIMARY KEY, reason TEXT)")
    await DB_CONN.execute("CREATE TABLE IF NOT EXISTS profiles(user_id INTEGER PRIMARY KEY, real_balance REAL DEFAULT 0, bonus_balance REAL DEFAULT 0, referrer_id INTEGER DEFAULT NULL, referrals_count INTEGER DEFAULT 0, subscribed INTEGER DEFAULT 0)")
    await DB_CONN.execute("CREATE TABLE IF NOT EXISTS promocodes(code TEXT PRIMARY KEY, amount REAL NOT NULL, activations INTEGER NOT NULL)")
    await DB_CONN.execute("CREATE TABLE IF NOT EXISTS promocode_uses(code TEXT NOT NULL, user_id INTEGER NOT NULL, PRIMARY KEY(code, user_id))")
    await DB_CONN.execute("CREATE TABLE IF NOT EXISTS referrals(invited_id INTEGER PRIMARY KEY, referrer_id INTEGER, rewarded INTEGER DEFAULT 0)")
    ...
    await migrate_db()
    async with DB_LOCK:
        defaults = [("show_discount", "1"), ("max_stars", str(DEFAULT_MAX_STARS)), ("min_stars", str(DEFAULT_MIN_STARS)), ("exchange_rate", str(DEFAULT_EXCHANGE_RATE)), ("auto_rate", "0")]
        for k, v in defaults:
            cur = await DB_CONN.execute("SELECT 1 FROM settings WHERE key = ? LIMIT 1", (k,))
...
async def migrate_db():
    ...
    async def table_exists(table: str) -> bool:
        cur = await DB_CONN.execute("SELECT name FROM sqlite_master WHERE type='table' AND name=? LIMIT 1", (table,))
    ...
    if await table_exists("profiles"):
        ...
        for name, ddl in {"real_balance": "real_balance REAL DEFAULT 0", "bonus_balance": "bonus_balance REAL DEFAULT 0", "referrer_id": "referrer_id INTEGER DEFAULT NULL", "referrals_count": "referrals_count INTEGER DEFAULT 0", "subscribed": "subscribed INTEGER DEFAULT 0"}.items():
            if name not in cols:
                await add_col("profiles", ddl)
    if await table_exists("orders"):
        cols = await existing_columns("orders")
        for name, ddl in {"item_type": "item_type TEXT", "item_id": "item_id TEXT", "months": "months INTEGER DEFAULT 0", "qty": "qty INTEGER DEFAULT 0", "price_rub": "price_rub REAL DEFAULT 0", "price_usd": "price_usd REAL DEFAULT 0", "price_stars": "price_stars INTEGER DEFAULT 0", "method": "method TEXT", "payment_ref": "payment_ref TEXT", "paid": "paid INTEGER DEFAULT 0", "created_at": "created_at TEXT DEFAULT CURRENT_TIMESTAMP"}.items():
            if name not in cols:
                await add_col("orders", ddl)
    if await table_exists("topups"):
        cols = await existing_columns("topups")
        for name, ddl in {"payment_ref": "payment_ref TEXT", "paid": "paid INTEGER DEFAULT 0", "created_at": "created_at TEXT DEFAULT CURRENT_TIMESTAMP"}.items():
            if name not in cols:
                await add_col("topups", ddl)
    if await table_exists("discounts"):
        cols = await existing_columns("discounts")
        if "id" not in cols:
            await DB_CONN.execute("CREATE TABLE IF NOT EXISTS discounts__new(id INTEGER PRIMARY KEY AUTOINCREMENT, title TEXT, percent REAL)")
            try:
                await DB_CONN.execute("INSERT INTO discounts__new(title, percent) SELECT title, percent FROM discounts")
            except Exception:
                pass
...
async def add_user(user_id: int, username: Optional[str], first_name: Optional[str]):
    async with DB_LOCK:
        await DB_CONN.execute("INSERT OR REPLACE INTO users(user_id, username, first_name) VALUES(?,?,?)", (user_id, username, first_name))
        await DB_CONN.execute("INSERT OR IGNORE INTO profiles(user_id) VALUES(?)", (user_id,))
        await DB_CONN.commit()
...
async def get_username_display(user_id: int) -> str:
    async with DB_LOCK:
        cur = await DB_CONN.execute("SELECT username, first_name FROM users WHERE user_id = ? LIMIT 1", (user_id,))
...
async def is_admin(user_id: int) -> bool:
    if user_id == MAIN_ADMIN_ID:
        return True
    async with DB_LOCK:
        cur = await DB_CONN.execute("SELECT 1 FROM admins WHERE user_id = ? LIMIT 1", (user_id,))
        return (await cur.fetchone()) is not None

async def add_admin_db(user_id: int):
    async with DB_LOCK:
        await DB_CONN.execute("INSERT OR REPLACE INTO admins(user_id) VALUES(?)", (user_id,))
        await DB_CONN.commit()
...
async def get_ban_reason(user_id: int) -> Optional[str]:
    async with DB_LOCK:
        cur = await DB_CONN.execute("SELECT reason FROM bans WHERE user_id = ? LIMIT 1", (user_id,))
        row = await cur.fetchone()
...
async def ban_user_db(user_id: int, reason: str):
    async with DB_LOCK:
        await DB_CONN.execute("INSERT OR REPLACE INTO bans(user_id, reason) VALUES(?,?)", (user_id, reason))
        await DB_CONN.commit()
...
async def get_profile(user_id: int) -> Dict[str, Any]:
    async with DB_LOCK:
        cur = await DB_CONN.execute("SELECT user_id, real_balance, bonus_balance, referrer_id, referrals_count, subscribed FROM profiles WHERE user_id = ? LIMIT 1", (user_id,))
        row = await cur.fetchone()
    if not row:
        return {"user_id": user_id, "real_balance": 0.0, "bonus_balance": 0.0, "referrer_id": None, "referrals_count": 0, "subscribed": 0}
    return {"user_id": row[0], "real_balance": float(row[1] or 0.0), "bonus_balance": float(row[2] or 0.0), "referrer_id": row[3], "referrals_count": int(row[4] or 0), "subscribed": int(row[5] or 0)}

async def add_real_balance(user_id: int, amount: float):
    async with DB_LOCK:
        await DB_CONN.execute("UPDATE profiles SET real_balance = real_balance + ? WHERE user_id = ?", (float(amount), user_id))
        await DB_CONN.commit()
...
async def set_money_balance(user_id: int, new_value: float):
    async with DB_LOCK:
        await DB_CONN.execute("UPDATE profiles SET real_balance = ? WHERE user_id = ?", (float(new_value), user_id))
        await DB_CONN.commit()
...
async def admin_add_money(user_id: int, amount: float, reason: str = ""):
    ...
    txt = f"💰 Баланс пополнен на <b>{amount:.2f}₽</b>. Текущий баланс: <b>{newb:.2f}₽</b>."
    if reason:
        txt += f"\nПричина: {reason}"
    try:
...
async def admin_sub_money(user_id: int, amount: float, reason: str = "") -> float:
    ...
    newb = (await get_profile(user_id))["real_balance"]
    txt = f"💸 С вашего баланса списано <b>{deduct:.2f}₽</b>. Текущий баланс: <b>{newb:.2f}₽</b>."
    if reason:
        txt += f"\nПричина: {reason}"
    try:
        await bot.send_message(user_id, txt)
...
async def set_profile_referrer(invited_id: int, referrer_id: int):
    async with DB_LOCK:
        await DB_CONN.execute("UPDATE profiles SET referrer_id = ? WHERE user_id = ? AND referrer_id IS NULL", (referrer_id, invited_id))
        await DB_CONN.execute("INSERT OR IGNORE INTO referrals(invited_id, referrer_id, rewarded) VALUES(?,?,0)", (invited_id, referrer_id))
        await DB_CONN.commit()

async def is_referral_rewarded(invited_id: int) -> bool:
    async with DB_LOCK:
        cur = await DB_CONN.execute("SELECT rewarded FROM referrals WHERE invited_id = ? LIMIT 1", (invited_id,))
        row = await cur.fetchone()
    return bool(row and int(row[0]) == 1)

async def mark_referral_rewarded(invited_id: int):
    async with DB_LOCK:
        await DB_CONN.execute("UPDATE referrals SET rewarded = 1 WHERE invited_id = ?", (invited_id,))
        cur = await DB_CONN.execute("SELECT referrer_id FROM referrals WHERE invited_id = ? LIMIT 1", (invited_id,))
        row = await cur.fetchone()
        if row and row[0]:
...
async def create_promocode(code: str, amount: float, activations: int):
    ...
    async with DB_LOCK:
        await DB_CONN.execute("INSERT OR REPLACE INTO promocodes(code, amount, activations) VALUES(?,?,?)", (code, float(amount), int(activations)))
        await DB_CONN.commit()
...
async def get_promocode(code: str) -> Optional[Dict[str, Any]]:
    code = (code or "").strip().upper()
    async with DB_LOCK:
        cur = await DB_CONN.execute("SELECT code, amount, activations FROM promocodes WHERE code = ? LIMIT 1", (code,))
        row = await cur.fetchone()
    if not row:
        return None
...
async def use_promocode(code: str, user_id: int) -> Optional[float]:
    ...
    if not code:
        return None
    async with DB_LOCK:
        cur = await DB_CONN.execute("SELECT amount, activations FROM promocodes WHERE code = ? LIMIT 1", (code,))
        row = await cur.fetchone()
        if not row:
            return None
        ...
        if activations <= 0:
            return None
        cur = await DB_CONN.execute("SELECT 1 FROM promocode_uses WHERE code = ? AND user_id = ? LIMIT 1", (code, user_id))
        if await cur.fetchone():
            return None
        await DB_CONN.execute("INSERT INTO promocode_uses(code, user_id) VALUES(?,?)", (code, user_id))
        await DB_CONN.execute("UPDATE promocodes SET activations = activations - 1 WHERE code = ?", (code,))
        await DB_CONN.commit()
        await add_real_balance(user_id, amount)
        return amount
...
async def create_discount(title: str, percent: float):
    async with DB_LOCK:
        await DB_CONN.execute("INSERT INTO discounts(title, percent) VALUES(?,?)", (title, float(percent)))
        await DB_CONN.commit()

async def delete_discount(id_or_title: str) -> bool:
    async with DB_LOCK:
        if id_or_title.isdigit():
            cur = await DB_CONN.execute("DELETE FROM discounts WHERE id = ?", (int(id_or_title),))
        else:
            cur = await DB_CONN.execute("DELETE FROM discounts WHERE title = ?", (id_or_title,))
        await DB_CONN.commit()
        return cur.rowcount > 0
...
async def list_discounts() -> List[Tuple[int, str, float]]:
    try:
        ...
        async with DB_LOCK:
            cur = await DB_CONN.execute("SELECT rowid AS id, title, percent FROM discounts ORDER BY rowid DESC")
            return await cur.fetchall()

async def get_setting(key: str, default: Optional[str] = None) -> Optional[str]:
    async with DB_LOCK:
        cur = await DB_CONN.execute("SELECT value FROM settings WHERE key = ? LIMIT 1", (key,))
        row = await cur.fetchone()
...
async def save_order(order_id: str, user_id: int, item_type: str, item_id: str = "", months: int = 0, qty: int = 0, price_rub: float = 0.0, price_usd: float = 0.0, price_stars: int = 0, method: str = "", payment_ref: Optional[str] = None):
    async with DB_LOCK:
        await DB_CONN.execute("INSERT OR REPLACE INTO orders(order_id, user_id, item_type, item_id, months, qty, price_rub, price_usd, price_stars, method, payment_ref, paid) VALUES(?,?,?,?,?,?,?,?,?,?,?,0)", (order_id, user_id, item_type, item_id, months, qty, float(price_rub), float(price_usd), int(price_stars), method, payment_ref))
        await DB_CONN.commit()

async def mark_order_paid(order_id: str) -> bool:
    async with DB_LOCK:
        cur = await DB_CONN.execute("UPDATE orders SET paid = 1 WHERE order_id = ?", (order_id,))
        await DB_CONN.commit()
        return cur.rowcount > 0

async def get_order(order_id: str) -> Optional[Tuple]:
    async with DB_LOCK:
        cur = await DB_CONN.execute("SELECT order_id, user_id, item_type, item_id, months, qty, price_rub, price_usd, price_stars, method, payment_ref, paid, created_at FROM orders WHERE order_id = ? LIMIT 1", (order_id,))
        return await cur.fetchone()

async def list_orders(limit: int = 200) -> List[Tuple]:
    async with DB_LOCK:
        cur = await DB_CONN.execute("SELECT order_id, user_id, item_type, item_id, months, qty, price_rub, price_usd, price_stars, method, payment_ref, paid, created_at FROM orders ORDER BY created_at DESC LIMIT ?", (limit,))
        return await cur.fetchall()

async def get_unpaid_topups(limit: int = 120) -> List[Tuple[str, int, float]]:
    async with DB_LOCK:
        cur = await DB_CONN.execute("SELECT order_id, user_id, price_rub FROM orders WHERE item_type = 'topup' AND paid = 0 ORDER BY created_at DESC LIMIT ?", (limit,))
        return await cur.fetchall()
...
def quickpay_link(receiver: str, amount_rub: float, label: str, payment_type: str = "AC") -> str:
    ...
    params = {"receiver": receiver, "quickpay-form": "shop", "sum": f"{amount_rub:.2f}", "paymentType": payment_type, "label": label}
...
async def yoomoney_check_label(label: str) -> bool:
    ...
    try:
        async with HTTP_SESSION.post(YOOMONEY_OP_HISTORY, data=data, headers=headers, timeout=15) as resp:
            if resp.status != 200:
                return False
            j = await resp.json()
            for op in j.get("operations", []):
                ...
                if str(lab) == str(label) and status in ("success", "completed", "done"):
                    return True
    except Exception:
...
def set_user_state(uid: int, state: Optional[str], tmp: Optional[str] = None, extra: Optional[Dict[str, Any]] = None):
...
async def cmd_start(message: Message):
    ...
    if args and args.startswith("ref"):
        try:
            ...
            pass
    subscribed = await check_subscription_status(message.from_user.id)
    async with DB_LOCK:
    ...
    if os.path.exists(BANNER_FILE):
        try:
            await bot.send_photo(chat_id=message.chat.id, photo=InputFile(BANNER_FILE), caption=caption, reply_markup=rk_main())
        except:
...
async def cmd_admin(message: Message):
    ...
    kb = InlineKeyboardMarkup(inline_keyboard=[
        [InlineKeyboardButton(text='📣 Рассылка', callback_data='admin_broadcast'), InlineKeyboardButton(text='📦 Просмотр заказов', callback_data='admin_view_orders')],
        [InlineKeyboardButton(text='🔎 Проверить платёж', callback_data='admin_check_payment'), InlineKeyboardButton(text='✅ Отметить оплату', callback_data='admin_mark_paid')],
        [InlineKeyboardButton(text='💸 Управление скидками', callback_data='admin_discounts'), InlineKeyboardButton(text='⛔ Бан/Разбан', callback_data='admin_ban')],
        [InlineKeyboardButton(text='💼 Балансы', callback_data='admin_balances')],
        [InlineKeyboardButton(text=f'🔔 Показать скидку: {show_label}', callback_data='admin_toggle_discount_display')],
        [InlineKeyboardButton(text=f'⭐ Макс звёзд: {max_stars}', callback_data='admin_set_max_stars'), InlineKeyboardButton(text=f'⭐ Мин звёзд: {min_stars}', callback_data='admin_set_min_stars')],
...
async def cb_admin_panel(callback: CallbackQuery):
    ...
    if data == 'admin_broadcast':
        ...
        for r in rows[:120]:
            ...
            status = "✅" if paid else "⏳"
            lines.append(f"{status} {oid} — {itype}:{iid or '-'} — {months}м/{qty}шт — {rub}₽ — {method} — ref:{pref or '-'} — user:{uid}")
        ...
        kb = InlineKeyboardMarkup(inline_keyboard=[[InlineKeyboardButton(text='➕ Добавить скидку', callback_data='discount_add')],[InlineKeyboardButton(text='➖ Удалить скидку', callback_data='discount_remove')],[InlineKeyboardButton(text='📋 Список скидок', callback_data='discount_list')],[InlineKeyboardButton(text='🔙 Назад', callback_data='admin_back')]])
        ...
        await callback.message.edit_text("✏️ Введите новое значение максимума звёзд (целое число), например: 150000")
    elif data == 'admin_set_min_stars':
        set_admin_state(callback.from_user.id, "waiting_set_min_stars")
        await callback.message.edit_text("✏️ Введите новое значение минимума звёзд (целое число), например: 300")
    elif data == 'admin_set_rate':
        set_admin_state(callback.from_user.id, "waiting_set_rate")
        await callback.message.edit_text("✏️ Введите новый коэффициент (число), например: 85.0")
...
async def cb_admin_submenus(callback: CallbackQuery):
    ...
    if data == "discount_add":
        ...
        await callback.message.edit_text("✏️ Введите: <b>Название %</b>\n\nНапример: <code>Осень 10</code> или просто <code>10</code>")
    elif data == "discount_remove":
        set_admin_state(callback.from_user.id, "waiting_discount_remove")
        await callback.message.edit_text("✏️ Введите <b>ID</b> или <b>Название</b> скидки для удаления:")
        ...
        await callback.message.edit_text("Введите: <b>КОД сумма активаций</b>\n\nПример: <code>SUPER 50 10</code>")
        ...
        async with DB_LOCK:
            cur = await DB_CONN.execute("SELECT code, amount, activations FROM promocodes ORDER BY code LIMIT 100")
            rows = await cur.fetchall()
        if not rows:
...
async def open_proxy(message: Message):
    ...
    await message.answer(f"🛰 <b>Proxy/VPN</b>\n\n<b>Технология</b>: {PROXY_TECH_DESCRIPTION}\n<b>Цена</b>: {PROXY_PRICE_RUB:.0f}₽ за любой.\n\nВсе — IPv4 Socks5.", reply_markup=rk_proxy())

async def open_free(message: Message):
    set_user_state(message.from_user.id, "free")
    await message.answer(FREE_PREMIUM_TEXT, reply_markup=ReplyKeyboardMarkup(keyboard=[[KeyboardButton(text=BTN_BACK)]], resize_keyboard=True))
...
async def open_topup_menu(message: Message):
    ...
    await message.answer("💳 <b>Пополнение Баланса</b>\n\nВыберите сумму или задайте свою:", reply_markup=rk_topup())
...
async def start_topup(message: Message, amount_rub: float):
    ...
    if not receiver:
        await message.answer("❗ Укажите номер кошелька YooMoney.", reply_markup=rk_profile())
        return
    link = quickpay_link(receiver, amount_rub, order_id, "AC")
    await save_order(order_id, uid, "topup", "", 0, 0, price_rub=amount_rub, method="yoomoney_topup", payment_ref=order_id)
    set_user_state(uid, "await_payment", tmp=order_id, extra={"pay_kind": "topup"})
    stars_need = await stars_needed_for_rub(amount_rub)
    await message.answer(f"🧾 Пополнение: <b>{amount_rub:.2f}₽</b>\nНомер заказа: <code>{order_id}</code>\n\n🔗 YooMoney:\n{link}\n\n⭐ Или подарите ровно <b>{stars_need}⭐</b> (или немного больше) профилю <b>@{STARS_GIFT_USERNAME}</b>\nи укажите в комментарии номер заказа:\n<code>{order_id}</code>\n\nПосле оплаты нажмите «Проверить Оплату».", reply_markup=rk_payment_actions())

async def open_premium_periods(message: Message):
    set_user_state(message.from_user.id, "premium_period")
    await message.answer("💎 <b>Telegram Premium</b>\n\nВыберите период:", reply_markup=rk_premium_periods())
...
async def start_stars_flow(message: Message):
    ...
    await message.answer(f"💰 <b>Купить Звёзды</b>\n\nМинимум: <b>{min_stars}⭐</b>\nМаксимум: <b>{max_stars}⭐</b>", reply_markup=rk_stars_menu())
...
async def text_router(message: Message):
        ...
        if ast:
            ...
            if ast == "waiting_ban_user":
                try:
                    ...
                    await message.reply(f"✅ Пользователь <b>{u_id}</b> заблокирован навсегда.")
                    try:
                        await bot.send_message(u_id, f"⛔ Вы заблокированы навсегда.\nПричина: {reason}")
                    except:
                        pass
                except:
                    await message.reply("Формат: user_id причина")
            ...
            if ast == "waiting_unban_user":
                try:
                    ...
                    await message.reply(f"✅ Пользователь <b>{u_id}</b> разблокирован.")
                    try:
                        t = "✅ Вы разблокированы и снова можете пользоваться ботом."
                        if reason:
                            t += f"\nКомментарий: {reason}"
            ...
            if ast == "waiting_promo_add":
                try:
                    ...
                    await message.reply(f"✅ Промокод <b>{code.upper()}</b> создан: <b>{amount:.0f}₽</b>, активаций: <b>{activ}</b>")
                except:
                    await message.reply("Формат: КОД сумма активаций\n\nПример: SUPER 50 10")
                clear_admin_state(uid)
                return
            if ast == "waiting_promo_delete":
                code = txt.strip().upper()
                ok = await delete_promocode(code)
                await message.reply(f"✅ Промокод <b>{code}</b> удалён." if ok else "Код не найден.")
                clear_admin_state(uid)
                return
            if ast == "waiting_check_payment":
                ...
                await message.reply(f"Статус: <b>{'Оплачено' if status.get('paid') else 'Не оплачено'}</b>")
                clear_admin_state(uid)
                return
            if ast == "waiting_mark_paid":
                ...
                if ok:
                    ...
                    await notify_admins_paid(o[1] if o else 0, oid, amount_rub, "Manual")
            ...
            if ast == "waiting_discount_add":
                try:
                    ...
                    await message.reply(f"✅ Добавлено: <b>{title}</b> — <b>{percent:.0f}%</b>")
                except:
                    await message.reply("Формат: <Название> <Процент>\nНапр.: Осень 10")
                clear_admin_state(uid)
                return
            ...
            if ast == "waiting_bal_money_zero":
                try:
                    ...
                    await message.reply(f"✅ Баланс пользователя {u} обнулён (было {prev:.2f}₽).")
                except:
                    await message.reply("Формат: user_id [причина]")
                clear_admin_state(uid)
                return
    ...
    if st == "promo_wait":
        ...
        if amount:
            ...
            await message.answer("Промокод недействителен или уже использован.", reply_markup=rk_profile())
    ...
    if st == "topup":
        ...
        if txt == "✏️ Своя Сумма":
            ...
            await message.answer(f"Введите сумму в ₽ (минимум {MIN_TOPUP_RUB:.0f}).")
            return
    ...
    if st == "await_payment":
        ...
        if txt == BTN_PAY_BAL:
            ...
            if bool(o[11]):
                ...
                return
            amount_rub = float(o[6] or 0.0)
            ok = await try_pay_with_balance(uid, amount_rub)
            if not ok:
                await message.reply("Недостаточно средств на балансе.", reply_markup=rk_payment_actions())
                return
            await mark_order_paid(last_order)
            await message.answer("✅ Оплата с баланса прошла.\n\nНапишите в поддержку для получения услуги.", reply_markup=rk_main())
        ...
        if txt == BTN_CHECK:
            ...
            if status.get("paid"):
                ...
                await notify_admins_paid(uid, last_order, float(o[6] or 0.0), "YooMoney")
                clear_user_state(uid)
                return
            else:
...
async def successful_payment_handler(message: Message):
    ...
    try:
        ...
        pass
    amount_rub = float(o[6] or 0.0) if o else 0.0
    await notify_admins_paid(message.from_user.id, order_id, amount_rub, "Stars(Invoice)")
